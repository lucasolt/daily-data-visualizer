...
def build_phase_report(df: pd.DataFrame, registry=None) -> str:
    ...
    comportamento idêntico ao anterior. Todo score do registro herda
    primary="mean" + min_cov=0.20 (são propriedades de "ser score subjetivo").
    """
    ...
    # Fonte dos scores: registro se houver (com label/direção de lá), senão score_map.
    SCORE_MIN_COV = 0.20
    score_rows = []
    if rmap and rg and rg.scores(rmap):
        ...
        score_specs = [(c, l, d) for c, (l, d) in score_map.items() if c in df.columns]
    ...
    # lista de meds a considerar nas pills: registro se houver, senão MED_COLORS
    if rmap and rg and rg.meds(rmap):
        med_cols_pills = [c for c in rg.meds(rmap) if c not in MED_EXCLUDE]
    else:
        med_cols_pills = list(MED_COLORS.keys())
    ...
    def med_pills_html(ph, min_pct: float = PILL_MIN_PCT):
        ...
        parts = []
        for col_name in med_cols_pills:
            ...
            if col_name in MED_COLORS:
                ...
                label = (rmap.get(col_name, {}).get("label") if rmap else None) \
                        or dp.SCORE_LABELS.get(col_name, col_name)
                reg_color = rmap.get(col_name, {}).get("color") if rmap else ""
                bgc = reg_color or GENERIC_PILL[0]
                fgc = GENERIC_PILL[1]
            # mediana da dose nos dias de uso (dose>0); regime estável por fase → dose real
...
"""
    ...
    # doses dos principais meds (hardcoded — fora do escopo do registro)
    DOSE_ROWS = [
...
<div class="wrapper">
  ...
  <div class="legend">
    ...
    <span>· intensidade proporcional ao desvio entre fases · horários: mediana (principal), μ média · scores subjetivos: média (principal), md mediana</span>
  </div>
